...
const {isMobile} = useGeneralContext()
    ...
    whileInView={{
        backgroundImage: [
            "linear-gradient(to right, #4D8890, #2F6B72, #4D8890)", // Main color with darker teal for depth
            "linear-gradient(to right, #7abfbf, #4D8890, #7abfbf)", // Lighter gradient with a fresh, bright teal
            "linear-gradient(to right, #4D8890, #2F6B72, #4D8890)", // Back to the main color gradient
          ]
...
>
    {text}
                ...
                className={`text-center text-white mt-4 px-6
                ${!description ? 'text-md sm:text-2xl md:text-3xl' : 'text-md sm:text-lg md:text-xl'}
                opacity-0`}
